...
export const useRetentionLaunch = (): void => {
  ...
  // prune + reschedule once hydrated, and again whenever entitlement settles
  useEffect(() => {
    if (!hydrated) return;
  ...
  useEffect(() => {
    if (!hydrated || !navigationKey || !show || seen) return;
    if (segments[0] !== "(tabs)") return;
    useLaunchStore.getState().markDataWarningSeen();
    void SheetManager.show("data-warning");
  }, [hydrated, navigationKey, show, seen, segments]);
};
